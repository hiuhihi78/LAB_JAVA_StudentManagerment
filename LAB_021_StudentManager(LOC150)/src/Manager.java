...
class Manager {
    ...
    public static void create(ArrayList<Student> list) {
        ...
        while (true) {
            ...
            count = count + 1;
            if (count > 0) { // check added more than 10 students
    ...
    public static void updateOrDelete(ArrayList<Student> list) {
        ...
        if (choiceUpdateOrDelete.equalsIgnoreCase("u")) {// update
            ...
            boolean studentUpdateExisted = Validation.checkStudentExistedById(list, newId);
            if (studentUpdateExisted == true) { // student with same newID was existed
                String oldStudentName = GetValue.getStudentById(list, newId).getName();
                if (oldStudentName.equalsIgnoreCase(name) == true) { // input old name
                    //System.out.println("You continue keep old name!");
                } else { // input new name for this student 
                    boolean wantUpdateNewName = Validation.checkYesNo(
                            "Do you want to update new name for this student?(Y/N)");
                    if (wantUpdateNewName == true) { // set new name with all record with same id
                        for (Student objStudent : list) {
                            if (objStudent.getId().equalsIgnoreCase(newId)) {
    ...
    public static void report(ArrayList<Student> list) {
        ...
        for (Student student : list) {
            ...
            // check student have enrolled a course two or more times in list
